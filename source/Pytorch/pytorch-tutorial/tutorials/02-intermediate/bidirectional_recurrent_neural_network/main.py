...
class BiRNN(nn.Module):  # 定义双向循环神经网络类
    def __init__(self, input_size, hidden_size, num_layers, num_classes):  # 初始化方法
        super(BiRNN, self).__init__()  # 调用父类的初始化方法
        self.hidden_size = hidden_size  # 隐藏层大小
        self.num_layers = num_layers  # LSTM 层数
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True)  # 创建双向 LSTM
        self.fc = nn.Linear(hidden_size*2, num_classes)  # 创建全连接层，输出大小为类别数量，乘以 2 是因为是双向 LSTM
    
    def forward(self, x):  # 前向传播方法
        # Set initial states
        h0 = torch.zeros(self.num_layers*2, x.size(0), self.hidden_size).to(device)  # 设置初始隐藏状态，乘以 2 是因为是双向 LSTM
        c0 = torch.zeros(self.num_layers*2, x.size(0), self.hidden_size).to(device)  # 设置初始细胞状态
        
        # Forward propagate LSTM
        out, _ = self.lstm(x, (h0, c0))  # 前向传播 LSTM，out 的形状为 (batch_size, seq_length, hidden_size*2)
        
        # Decode the hidden state of the last time step
        out = self.fc(out[:, -1, :])  # 解码最后一个时间步的隐藏状态
        return out  # 返回输出

model = BiRNN(input_size, hidden_size, num_layers, num_classes).to(device)  # 创建模型并转移到指定设备
...
for epoch in range(num_epochs):  # 遍历每个训练轮
    for i, (images, labels) in enumerate(train_loader):  # 遍历训练数据
        images = images.reshape(-1, sequence_length, input_size).to(device)  # 将图像展平为 (batch_size, sequence_length, input_size)

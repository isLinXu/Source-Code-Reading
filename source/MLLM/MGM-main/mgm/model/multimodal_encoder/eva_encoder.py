...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = nn.Linear(in_features, hidden_features)  # 第一全连接层
        self.act = act_layer()  # 激活函数
        self.fc2 = nn.Linear(hidden_features, out_features)  # 第二全连接层
        self.drop = nn.Dropout(drop)  # 随机失活层
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        all_head_dim = head_dim * self.num_heads  # 总维度
        self.scale = qk_scale or head_dim ** -0.5  # 缩放因子

        # 初始化QKV投影矩阵
        self.qkv = nn.Linear(dim, all_head_dim * 3, bias=False)
        if qkv_bias:
            self.q_bias = nn.Parameter(torch.zeros(all_head_dim))  # Q偏置
            self.v_bias = nn.Parameter(torch.zeros(all_head_dim))  # V偏置
        else:
            self.q_bias = None
            self.v_bias = None
        ...
        if window_size:
            ...
            self.relative_position_bias_table = nn.Parameter(
                torch.zeros(self.num_relative_distance, num_heads))  # 相对位置偏置表
        ...
        self.attn_drop = nn.Dropout(attn_drop)  # 注意力随机失活
        self.proj = nn.Linear(all_head_dim, dim)  # 输出投影
        self.proj_drop = nn.Dropout(proj_drop)  # 投影随机失活
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        if self.relative_position_bias_table is not None:
            ...
            relative_position_bias = relative_position_bias.view(
                ...
                self.window_size[0] * self.window_size[1] + 1, -1).permute(2, 0, 1).contiguous()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()  # 随机路径丢弃
...
class PatchEmbed(nn.Module):
    """图像分块嵌入模块 Image to Patch Embedding"""
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)  # 卷积投影层
    ...
    def forward(self, x, **kwargs):
        ...
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"输入图像尺寸({H}*{W})与模型({self.img_size[0]}*{self.img_size[1]})不匹配"
        x = self.proj(x).flatten(2).transpose(1, 2)  # 展平并转置
        return x
...
class RelativePositionBias(nn.Module):
    ...
    def forward(self):
        """前向传播获取相对位置偏置"""
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)]
        return relative_position_bias.view(
            self.window_size[0] * self.window_size[1] + 1,
            self.window_size[0] * self.window_size[1] + 1, -1).permute(2, 0, 1).contiguous()


class VisionTransformer(nn.Module):
    """视觉Transformer主类 Vision Transformer with support for patch or hybrid CNN input stage"""
...
def interpolate_pos_embed(model, checkpoint_model):
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        if orig_size != new_size:
            print(f"位置编码插值: 从 {orig_size}x{orig_size} 到 {new_size}x{new_size}")
            # 分离额外token（CLS等）
...
def convert_weights_to_fp16(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        if isinstance(l, (nn.Conv1d, nn.Conv2d, nn.Linear)):
            l.weight.data = l.weight.data.half()  # 权重转半精度
            if l.bias is not None:
                l.bias.data = l.bias.data.half()  # 偏置转半精度
...
class EVAVisionTower(nn.Module):
    ...
    def feature_select(self, image_features):
        ...
        if self.select_feature == 'patch':
            ...
            raise ValueError(f'未知特征选择方式: {self.select_feature}')
        return image_features

...
PATH_WEB_DOCS_S3 = f"s3://m4-datasets-us-east-1/webdocs/web_document_dataset_filtered_imgurldedup_nsfwfiltered_urldedup_linededup_finalcleaning_setimgurlsdedup_optoutrmv_finalprocessing_pplfilter_optoutrmvdoc2/{IDX_JOB}/web_docs_optoutrmv"
...
PATH_SAVE_S3_WEB_DOCS_OPT_OUT_FILTERED = f"s3://m4-datasets-us-east-1/webdocs/web_document_dataset_filtered_imgurldedup_nsfwfiltered_urldedup_linededup_finalcleaning_setimgurlsdedup_optoutrmv_finalprocessing_pplfilter_optoutrmvdoc2_optoutrmvimg2/{IDX_JOB}/"


class OptOutFilteringImages:
    ...
    def __call__(self, example):
        ...
        indices_to_remove = [
            ind for ind, meta in enumerate(metadata) if (meta is not None) and (meta["src"] in self.opt_out_urls)
        ]
...
if __name__ == "__main__":
    ...
    web_docs.reset_format()
    logger.info("Finished loading the web docs")

    logger.info("Starting removing the opt out images")
    opt_out_filtering_images = OptOutFilteringImages(path_opt_out_urls=PATH_OPT_OUT_URLS_LOCAL)
    web_docs = web_docs.remove_columns(["input_ids", "attention_mask"])
    web_docs = web_docs.map(opt_out_filtering_images, num_proc=NUM_PROC_FILTERING)

# coding=utf-8
# Copyright 2023 Meta and the HuggingFace Inc. team. All rights reserved.
#
...
_CONFIG_FOR_DOC = "VLlama3Config"


@dataclass
class VLlama3BaseModelOutputWithPast(ModelOutput):
    """
...
@dataclass
class VLlama3CausalLMOutputWithPast(ModelOutput):
    """
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[VLlama3Config] = None,
    ):
        super().__init__()
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: VLlama3Config, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: VLlama3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class VLlama3PreTrainedModel(VLOOMPreTrainedModelBase):
    config_class = VLlama3Config
...
class VLlama3Model(VLlama3PreTrainedModel):
    """
    ...
    def __init__(self, config: VLlama3Config, vision_model=None):
        super().__init__(config)
        self.config = config
        self.padding_idx = 2
        self.vocab_size = config.vocab_size
        self.config._attn_implementation = "flash_attention_2" if self.config._flash_attn_2_enabled else "eager"
        ...
        # init vision model
        vision_model_name = self.config.vision_config.vision_model_name
        ...
        #    input_size=self.config.vision_config.embed_dim * 4,
        #    intermediate_size=self.config.intermediate_size,
        #    output_size=self.config.hidden_size,
        # )
        self.modality_projection = SimpleMLP(
            input_size=self.config.vision_config.embed_dim * (self.config.pixel_shuffle_factor**2),
            output_size=self.config.hidden_size,
        )
        ...
        if config.use_resampler:
            ...
            self.image_seq_len = int(((config.vision_config.image_size // config.vision_config.patch_size) ** 2) // (self.config.pixel_shuffle_factor**2))
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, VLlama3BaseModelOutputWithPast]:
        ...
        if pixel_values is not None and image_hidden_states is not None:
            ...
            image_hidden_states = pixel_shuffle(image_hidden_states, scale_factor=self.config.pixel_shuffle_factor)
        ...
        if inputs_embeds is not None and image_hidden_states is not None and input_ids.shape[1] != 1:
        ...
        return VLlama3BaseModelOutputWithPast(
    ...
    def _update_causal_mask(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if sequence_length != 1:
                # torch.triu currently doesn't work with bfloat16, so we use a workaround
...
class VLlama3ForCausalLM(VLlama3PreTrainedModel):
    ...
    def __init__(self, config, vision_model=None):
        super().__init__(config)
        self.model = VLlama3Model(config, vision_model=vision_model)
    ...
    @add_start_docstrings_to_model_forward(LLAMA_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=VLlama3CausalLMOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        return_dict: Optional[bool] = None,
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, VLlama3CausalLMOutputWithPast]:
        r"""
        Args:
        ...
        return VLlama3CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
